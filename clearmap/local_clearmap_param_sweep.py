...
"outputdirectory": os.path.join(systemdirectory, "wang/zahra/kelly_cell_detection_analysis/comparison_to_clearmap/f37104_demonstrator_20171016"),
...
if __name__ == "__main__":
    ...
    arrayjob(0, cores=12, compression=1, **params) #process zslice numbers equal to slurmjobfactor*jobid thru (jobid+1)*slurmjobfactor
    arrayjob(1, cores=12, compression=1, **params) #process zslice numbers equal to slurmjobfactor*jobid thru (jobid+1)*slurmjobfactor
    ...
    #set param iter ranges here
    rBP_size_r = range(3,11,2) #[5, 11] #range(5,19,2) ###evens seem to not be good <-- IMPORTANT TO SWEEP
    fEMP_hmax_r = [None]#[None, 5, 10, 20, 40]
    fEMP_size_r = [20] #zmd added, <-- IMPORTANT TO SWEEP
    ...
    for jobid in range(tick): #to find the range value, run lines 152 - 173 in this script, part of sweep_parameters_cluster func
        sweep_parameters_cluster(jobid, rBP_size_r, fEMP_hmax_r, fEMP_size_r, fEMP_threshold_r, fIP_method_r, 
                                 fIP_size_r, dCSP_threshold_r, tick, **params)
