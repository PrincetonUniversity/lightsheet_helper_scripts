...
#SBATCH -t 300
#SBATCH -o logs/smartspim_inverse_reg_%j.out        # STDOUT #add _%a to see each array job
#SBATCH -e logs/smartspim_inverse_reg_%j.err        # STDERR #add _%a to see each array job
...
python spim_register_semicustom.py 1 ${src} ${reg} ${reg_vol} ${cell} ${cell_vol} ${atl}
