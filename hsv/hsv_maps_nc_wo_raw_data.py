...
#imports
#path to pickle file
data_pth = "/jukebox/wang/zahra/h129_contra_vs_ipsi/data/thal_hsv_maps_contra_allen.p"
data = pckl.load(open(data_pth, "rb"), encoding = "latin1")
...
#%%
nuclei = ["VPM", "VPL", "VA-L", "AV", "LD", "Paraventricular", 
          "M Habenula", "LP", "Post. Triangular", "MD", "Post. complex", "VM", "RTN"]

pcounts = np.nan_to_num(np.asarray([((brain[1:]/brain[0])*100) for brain in counts_per_struct.T]))    

#make % mean counts map

#only look at mean counts per "cerebellar region" (i.e. that which had the highest contribution of the injection)    
mean_counts = np.asarray([np.nanmean(pcounts[np.where(primary_pool == idx)[0]], axis=0) for idx in np.unique(primary_pool)])
...
ax.set_yticklabels(["{}".format(bi) for bi in nuclei], fontsize="small")
dst = "/home/wanglab/Desktop"
#%%
...
plt.savefig(os.path.join(dst, "hsv_density_thal.pdf"), bbox_inches = "tight")