...
# -*- coding: utf-8 -*-
"""
Created on Sat May 18 14:15:37 2019

@author: wanglab
"""

import statsmodels.api as sm
import pickle as pckl, numpy as np, seaborn as sns, pandas as pd, os
...
primary_lob_n = np.asarray([np.where(primary_pool == i)[0].shape[0] for i in np.unique(data["primary_pool"])])
...
cell_counts_per_brain_pool = np.asarray([[brain[0], brain[1]+brain[2], brain[3], brain[4], 
                                          brain[5], brain[6], brain[7], brain[8], brain[9] ,brain[10],
                                          brain[11], brain[12]] for brain in cell_counts_per_brain])

regions = np.asarray(['Ventral group of the dorsal thalamus',
       'Subparafascicular nucleus & area', 'Peripeduncular nucleus', 'Geniculate group, dorsal thalamus',
...
#%%
#from badura et al.
#shuffle doesnt coverge for glm
