...
fig, axs = plt.subplots(4, 9, sharex='col', sharey='row',
...
for ri, row in enumerate(axs):
    for ci, col in enumerate(row):
        try:
            img = col.imshow(np.rot90(np.max(atl_raw[:, 423:, :], axis=1), axes=(1,0)), cmap = "gist_yarg")
            img = col.imshow(np.rot90(np.max(inj_raw[i].astype(int), axis=1), axes=(1,0)), cmap = my_cmap, alpha = 0.8)
            col.text(.5, .5, "{}".format(brains[i]), fontsize=3, ha='left', va='top', transform=col.transAxes)
            col.text(.1, .1, "lob i-v: {}\nlob vi-vii: {}\nlob viii-x: {}\nsim: {}\ncr1, cr2: {}\npm, cp: {}".format(round(expr_all_as_frac_of_lob_pool[i][0], 2),
                                                             round(expr_all_as_frac_of_lob_pool[i][1], 2),
                                                            round(expr_all_as_frac_of_lob_pool[i][2], 2),
                                                             round(expr_all_as_frac_of_lob_pool[i][3], 2),
                                                               round(expr_all_as_frac_of_lob_pool[i][4], 2),
                                                               round(expr_all_as_frac_of_lob_pool[i][5], 2)), fontsize=5, 
...
#np.save("/home/wanglab/mounts/wang/zahra/modeling/h129/neocortex/normalised_counts.npy", cell_counts_per_brain_pool)        
#injp = np.nan_to_num(normalised)
injp = np.nan_to_num(normalised[mask, ...])
#np.save("/home/wanglab/mounts/wang/zahra/modeling/h129/neocortex/normalised_inj.npy", injp)
#%%
...
for y,x in np.argwhere(sig):
    pass
    ax.text(x, y+0.7, "*", fontsize=10, ha="left", va="bottom", color = "black", transform=ax.transData)
