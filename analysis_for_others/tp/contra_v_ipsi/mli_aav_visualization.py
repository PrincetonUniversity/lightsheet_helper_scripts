...
if __name__ == "__main__":
    ...
    src = "/jukebox/wang/pisano/tracing_output/aav"
    dst = "/home/wanglab/Desktop"
    
    #animals = ["an01", "an02", "an03", "an04", "an05", "an06", "an07", "an09", "an10", "an12", "an13", "an14", "an15", "an16", "an17"]
    animals = [#"20171130_pcp2_dcn_02", 
               #"20171130_pcp2_dcn_03"]
               "20171130_pcp2_dcn_07"]
    ...
    for brain in brains:
        ...
        for i in range(ncols):
            for j in range(nrows):
                axes[i,j].pcolormesh(np.max(vol_cor[slcs[k]:slcs[k]+maxip_scale]*45, axis=0), cmap="Greys")
        ...
        for i in range(ncols):
            for j in range(nrows):
                axes[i,j].pcolormesh(np.max(vol_cor[slcs[k]:slcs[k]+maxip_scale]*40, axis=0), cmap="Greys")
                axes[i,j].invert_yaxis()
                axes[i,j].axis("off")
                k += 1        
