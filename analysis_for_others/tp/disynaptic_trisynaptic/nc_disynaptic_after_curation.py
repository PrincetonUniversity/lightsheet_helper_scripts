...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug 14 11:59:26 2019
...
for k,v in cells_pooled_regions.items():
    ...
    for j,l in dct.items():
        i.append(l)  
    thal_cell_counts_per_brain.append(i)
    #re-initialise for next
    i = []  
    
thal_cell_counts_per_brain = np.asarray(thal_cell_counts_per_brain)

thal_volume_per_brain = []
...
for k,v in volume_pooled_regions.items():
    ...
    for j,l in dct.items():
        i.append(l)  
    thal_volume_per_brain.append(i)
    #re-initialise for next
    i = []  
    
thal_volume_per_brain = np.asarray(thal_volume_per_brain)*(scale_factor**3)

#calculate denisty
thal_density_per_brain = np.asarray([xx/thal_volume_per_brain[i] for i, xx in enumerate(thal_cell_counts_per_brain)])
...
#%%
#NC
nc_brains = ["20180409_jg46_bl6_lob6a_04",
 "20180608_jg75",
...
for k,v in cells_pooled_regions.items():
    ...
    for j,l in dct.items():
        i.append(l)  
    nc_cell_counts_per_brain.append(i)
    #re-initialise for next
    i = []  
    
nc_cell_counts_per_brain = np.asarray(nc_cell_counts_per_brain)
...
for k,v in volume_pooled_regions.items():
    ...
    for j,l in dct.items():
        i.append(l)  
    nc_volume_per_brain.append(i)
    #re-initialise for next
    i = []  
    
nc_volume_per_brain = np.asarray(nc_volume_per_brain)*(scale_factor**3)

#calculate denisty
nc_density_per_brain = np.asarray([xx/nc_volume_per_brain[i] for i, xx in enumerate(nc_cell_counts_per_brain)])
