...
csv_pth = r"C:\Users\Zahra\Desktop\select_structures_percent_counts_for_visualization.csv"

cfos = pd.read_csv(csv_pth)

#look at certain groups
cfos = cfos[cfos.group == "CNO_control_no_reversal"]

#look at nc first?
#ncs = ["Somatosensory areas", "Gustatory areas",
#       "Visceral area", "Auditory areas", "Visual areas",
#       "Anterior cingulate area", "Prelimbic area", "Infralimbic area",
#       "Orbital area", "Agranular insular area", "Retrosplenial area",
#       "Posterior parietal association areas", "Temporal association areas",
#       "Perirhinal area", "Ectorhinal area"]
...
#       "Lateral septal complex", "Striatum-like amygdalar nuclei", "Pallidum"]
#
...
##cortical subplate
#ncs = ["Taenia tecta",
#       "Cortical amygdalar area","Hippocampal region", "Retrohippocampal region"]

#ncs = [xx for xx in list(cfos.columns) if not xx == "animal" 
#       and not xx == "group" and cfos[xx][0] > 0]

#x
counts = np.asarray([cfos[nc].values for nc in ncs])
#group?
 
ymaze_pth = r"C:\Users\Zahra\Desktop\ymaze.csv"

ym = pd.read_csv(ymaze_pth, index_col = None)

#look at certain groups
ym = ym[(ym.group == "CNO Control") & (ym.Condition == "no rev")]

mts = ["Initial Learning",
       "Multisession Learning", "Final Learning", "Initial Reversal",
       "Multisession Reversal ", "Final Reversal", "Ymaze Distance Traveled"]
...
#dendogram analysis
df = pd.DataFrame(X, cfos["animal"].values)

df.columns = ncs

vmax=1
import copy
my_cmap = copy.copy(plt.cm.magma)
my_cmap.set_over("orange")

sns.set(font_scale=0.7)
sns.clustermap(df.T, cmap = my_cmap, vmax = vmax, standard_scale = 0, metric="correlation")
sns.axes_style({ "patch.force_edgecolor": True})
plt.savefig("/home/wanglab/Desktop/counts_sort.pdf", bbox_inches = "tight")
#%%
#from badura et al.
mat = []
pmat = []
