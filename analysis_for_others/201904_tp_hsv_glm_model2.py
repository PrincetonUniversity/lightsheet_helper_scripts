...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 11:05:42 2019
...
ax.set_yticklabels(["{}".format(bi) for ar,bi in zip(ars, regions)], fontsize="x-small")
...
for rg in rchoices:
    ...
    if not np.nonzero(b_vec)[0].shape == (0,):
        ...
        plt.savefig('/home/wanglab/Desktop/figs/scatter_{}_{}.pdf'.format(rg, echoice))
...
for rname, reh in zip(regions, cell_counts_per_brain.T):
    if not np.nonzero(reh)[0].shape == (0,):
        ...
        for lob,lobname,ax in zip(inj.T, ak, axs.ravel()):
            ...
            ax.set_xticks([])
            ax.set_yticks([])
        plt.figtext(.5, .05, "Fraction of area injected", fontsize="small")
