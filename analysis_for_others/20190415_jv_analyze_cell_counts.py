...
def generate_percent_counts_and_density_per_region(src, csv_pth):
    ...
    df["volume"] = df[df.voxels_in_structure > 0].apply(lambda x: x.voxels_in_structure*scale, 1)
    df["density (cells/mm3)"] = df[df.voxels_in_structure > 0].apply(lambda x:x.counts/(float(x.voxels_in_structure*scale)), 1)
    #export
