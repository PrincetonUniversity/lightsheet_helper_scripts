...
flds = [os.path.join(pth, "param_sweep/"+xx) for xx in os.listdir(os.path.join(pth, "param_sweep"))]
...
for n, fld in enumerate(flds):
    ...
    measures = [pairwise_distance_metrics(annotated_cells[i], detected_cells[i], cutoff = cutoff, verbose = False) for i in range(len(arr_pths))] 
...
df.to_csv(os.path.join(pth, "clearmap_performance_measures_kelly_cfos.csv"))
