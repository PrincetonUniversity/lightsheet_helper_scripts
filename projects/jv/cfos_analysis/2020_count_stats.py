# -*- coding: utf-8 -*-
"""
Created on Thu Mar 26 13:15:45 2020
...
main_df.to_csv(os.path.join(dst, "select_structures_percent_counts.csv"))
...
struct = [list(itertools.repeat(xx, 33)) for xx in main_df.columns.values if xx != "group"]
struct = pd.Series(list(itertools.chain.from_iterable(struct)))
rotate_df["name"] = struct

vals = [pd.Series(main_df[xx].values) for xx in main_df.columns.values if xx != "group"]    
