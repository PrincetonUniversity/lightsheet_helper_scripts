...
post_transformed = [os.path.join(src, os.path.join(xx, "transformed_points")) for xx in brains]

####################################ONLY NEED TO RUN ONCE OR WHEN DOUBLE CHECKING!!!!!!!!############################################
...
right = transformed_cells_to_ann(post_transformed, ann_right, dst, "nc_right_side_no_prog_at_each_level_pma_test.p")
#collect counts from left side
left = transformed_cells_to_ann(post_transformed, ann_left, dst, "nc_left_side_no_prog_at_each_level_pma_test.p")

####################################ONLY NEED TO RUN ONCE OR WHEN DOUBLE CHECKING!!!!!!!!############################################

#import dict of cells by region
r_cells_regions = pckl.load(open(os.path.join(dst, "nc_right_side_no_prog_at_each_level_pma_test.p"), "rb"), encoding = "latin1")
...
#LEFT SIDE
l_cells_regions = pckl.load(open(os.path.join(dst, "nc_left_side_no_prog_at_each_level_pma_test.p"), "rb"), encoding = "latin1")
...
contra_df.to_csv(os.path.join(dst, "data/nc_contra_counts_33_brains_pma_test.csv")) 

ipsi_df = pd.DataFrame(ipsi)
ipsi_df.to_csv(os.path.join(dst, "data/nc_ipsi_counts_33_brains_pma_test.csv"))         
#%%
cells_regions_pth = os.path.join(dst, "data/nc_contra_counts_33_brains_pma_test.csv")
