...
if __name__ == "__main__":
    ...
    rawdata_path = os.path.join('/jukebox/scratch/zmd',f'an{animal_id}',
        'transformed_annotations','single_tifs')
    assert os.path.exists(rawdata_path)
    layer_name = f'rawatlas_an{animal_id}'
    ...
    """ Raw atlas has the same dimensions as raw data """
    x_dim = 2160
    y_dim = 2560
   
    all_slices = glob.glob(f"{rawdata_path}/*annotation*tif")  
    ...
    if step == 'step0':
        ...
        vol = make_info_file(volume_size=volume_size,
            layer_dir=layer_dir,resolution=resolution,atlas_type='Princeton')
    elif step == 'step1':
        print("step 1")
