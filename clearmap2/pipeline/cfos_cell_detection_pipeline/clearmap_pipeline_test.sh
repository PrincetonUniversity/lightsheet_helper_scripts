...
do
	...
	--array=0-${max_array_job} spock-clearmap/slurm_scripts/cell_detect_test.sh)
	echo $OUT2
	
	# ## Merge the blocks
	OUT3=$(sbatch --parsable --dependency=afterok:${OUT2##* } \
	--export=ALL,sample_dir=${sample_dir},output_rootpath=${output_rootpath} \
	spock-clearmap/slurm_scripts/merge_blocks_test.sh)
	echo $OUT3
	...
	--export=ALL,sample_dir=${sample_dir},output_rootpath=${output_rootpath} \
	registration/slurm_scripts/spim_inverse_register_test.sh)
	echo $OUT5
	...
	--export=ALL,sample_dir=${sample_dir},output_rootpath=${output_rootpath} \
	spock-clearmap/slurm_scripts/postprocessing_test.sh)
	echo $OUT6
	...
	--export=ALL,sample_dir=${sample_dir},output_rootpath=${output_rootpath} \
	downsizing/diagnostic_postprocessing.sh)
