...
do
	...
	# OUT1=$(sbatch --parsable --export=ALL,request_name=$2 spock-clearmap/slurm_scripts/create_memmap_vol.sh)
	# echo $OUT1

	### Run the individual blocks in array jobs
	# OUT2=$(sbatch --parsable --dependency=afterok:${OUT1##* } \
	#--export=ALL,sample_dir=${sample_dir} --array= spock-clearmap/slurm_scripts/cell_detect.sh)
	# echo $OUT1
done
